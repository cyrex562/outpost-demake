...
void __stdcall16far Build_Site_Object_Setup_GDI_Context(void *obj_ptr, int res_id, int width, int height)
{
    if (obj_ptr == NULL) return;
    astruct_437 *this = (astruct_437 *)obj_ptr;

    // 1. Load resources via index 5 in vtable.
    void **vtable = *(void ***)this;
    typedef void (__stdcall16far *LoadResFunc)(void *this, int res_id);
    ((LoadResFunc)vtable[5])(this, res_id);

    // 2. Setup Device Context (HDC).
    // field9_0xa is likely the internal bitmap.
    void *bmp = (void *)this->field9_0xa;
    if (bmp != NULL)
    {
        char *driver = (char *)get_with_lazy_init(bmp);
        // Bitmap_Object_Get_Internal_Buffer_Ptr_maybe
        void *buffer = Bitmap_Object_Get_Buffer_Ptr_maybe(bmp);
        
        // CreateDC16(driver, device, output, initData)
        this->field13_0x12 = CreateDC16(driver, NULL, NULL, NULL);
        
        // 3. Select and realize palette into the new DC.
        // Virtual call at index 2 of bitmap vtable.
        void **bmp_vtable = *(void ***)bmp;
        typedef HPALETTE16 (__stdcall16far *SelectPalFunc)(void *this, HDC16 *pHdc);
        this->field20_0x1a = (u32)((SelectPalFunc)bmp_vtable[2])(bmp, &this->field13_0x12);
    }

    // 4. Scale UI metrics if resolution exceeds 640x400 (0x280 x 0x1E0).
    if (u16_1050_422e != 0 && height > 640)
    {
        // Scaling tables at 0x4172 and 0x419A.
        short *scaleX = (short *)0x4172;
        short *scaleY = (short *)0x419A;
        
        for (int i = 0; i <= (int)u16_1050_4216; i++)
        {
            scaleX[i] = (short)((long)scaleX[i] * (height + 1) / 640);
        }
        
        for (int i = 0; i <= (int)u16_1050_422c; i++)
        {
            scaleY[i] = (short)((long)scaleY[i] * (width + 1) / 480);
        }
    }
          ...
          (undefined4 param_1,undefined2 param_2,undefined4 param_3)
{
  ...
  uStack_a = (astruct_433 *)CONCAT22(reg_dx,puVar2);
