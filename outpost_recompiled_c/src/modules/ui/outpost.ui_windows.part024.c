          ...
          (void *param_1,int param_2,uint param_3,uint param_4,u16 param_5)
{
  ...
  uStack_6 = (uint *)CONCAT22(reg_dx,reg_ax);
  uVar1 = reg_dx | (uint)reg_ax;
  if (uVar1 == 0x0)
  {
    UI_Item_List_Binary_Search_Logic(param_1,0x0);
    uVar2 = uVar1 | (uint)reg_ax;
    if (uVar2 == 0x0)
    {
      UI_Item_List_Expand_Capacity_Logic(param_1);
      UI_Item_List_Binary_Search_Logic(param_1,0x0);
      if (uVar2 == 0x0 && reg_ax == NULL)
      {
        return 0x0;
      }
      reg_ax[0x2] = param_5;
    ...
    if (param_2 != 0x0)
    {
      bVar3 = CARRY2(*reg_ax,param_3);
      param_3 = *reg_ax + param_3;
      param_4 = reg_ax[0x1] + param_4 + (uint)bVar3;
    }
    *reg_ax = param_3;
    reg_ax[0x1] = param_4;
