...
heap_create_pool(int flags,int initial_size,int limit,int bucket_size)
{
  ...
  undefined *reg_dx;
  undefined *puVar4;
  /* unaff_SI: Win16 callee-saved SI = initial segment half of far pool ptr; flat model = 0 */
  long lVar5;
  ulong uVar6;
  
  uVar6 = 0; /* unaff_SI << 16: Win16 segment half of pool far ptr — flat model: 0 */
  ...
  if (true)
  {
    do
    {
      ...
      reg_dx = (undefined *)((uint)puVar4 | (uint)ptr);
      if (reg_dx != NULL) break;
      uVar6 &= 0xffff0000;
      puVar2 = ptr;
...
long __cdecl16near heap_round_up_size(long size)
{
  ...
  {
LAB_1000_1afa:
    return CONCAT22(reg_dx,reg_ax);
  }
  if (reg_ax < 0xfff0)
  {
    if (reg_ax < 0x1001)
    {
      return CONCAT22(reg_dx,0x1000);
    }
    uVar1 = 0x2000;
    if (reg_ax < 0x2001)
    {
    ...
    while (uVar1 *= 0x2, uVar1 != 0x0)
    {
      if (reg_ax <= uVar1)
      {
        return CONCAT22(reg_dx,(uVar1 + 0x10 & -(uint)(uVar1 < 0xfff0)) - 0x10);
      }
    }
  }
  return CONCAT22(reg_dx,0xfff0);
}
