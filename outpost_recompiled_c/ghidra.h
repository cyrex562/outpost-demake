...
typedef uint32_t int3;   /* Ghidra 3-byte integer pseudo-type */
...
#define CONCAT12(a,b) ((((uint32_t)(uint8_t)(a)) << 16) | ((uint32_t)(uint16_t)(b)))
#define CONCAT13(a,b) ((((uint32_t)(uint8_t)(a)) << 24) | ((uint32_t)(b) & 0x00FFFFFFu))
#define CONCAT21(a,b) ((((uint32_t)(uint16_t)(a)) << 8)  | ((uint32_t)(uint8_t)(b)))
#define CONCAT22(a,b) ((((uint32_t)(uint16_t)(a)) << 16) | ((uint32_t)(uint16_t)(b)))
#define CONCAT31(a,b) ((((uint32_t)(a) & 0x00FFFFFFu) << 8) | ((uint32_t)(uint8_t)(b)))
...
#define SCARRY1(a,b) ((uint8_t)((uint16_t)(uint8_t)(a) + (uint16_t)(uint8_t)(b)) > 0xFF)
#define SCARRY2(a,b) CARRY2(a,b) // Simplified for initial build
#define SBORROW1(a,b) ((int8_t)(a) < (int8_t)(b))
...
// --- x86 Port I/O Stubs ---
// In flat Win32 model, direct port I/O is not used; stub to no-ops.
#define in(port)         ((undefined2)0)
#define out(port,val)    ((void)(port),(void)(val))

// --- Ghidra Software Interrupt Stub ---
// swi(n) emits 'code *' cast used as: pcVar1 = (code *)swi(0x21); (*pcVar1)();
// 'code' typedef is in missing_deps.h (typedef void (*code)(void)).
// In flat Win32 model, INT 21h is not callable; smoke build stubs to NULL.
#define swi(n) ((code)0)

// --- POPCOUNT macro (count of set bits) ---
// MSVC intrinsic __popcnt available; fall back to a portable constant if absent.
#ifdef _MSC_VER
#  include <intrin.h>
#  define POPCOUNT(x) __popcnt((unsigned)(x))
#else
